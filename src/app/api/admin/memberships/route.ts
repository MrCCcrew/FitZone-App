...
const daysToLabel = (d: number) =>
  d <= 31 ? "monthly" : d <= 100 ? "quarterly" : "annual";
const labelToDays = (l: string) =>
  l === "monthly" ? 30 : l === "quarterly" ? 90 : 365;

export async function GET() {
  const err = await checkAdmin(); if (err) return err;

  const rows = await db.membership.findMany({ orderBy: { name: "asc" } });

  const plans = await Promise.all(rows.map(async (m) => {
    ...
    return {
      ...
      features:     (() => { try { return JSON.parse(m.features); } catch { return []; } })(),
      active:       m.isActive,
      membersCount,
    };
  }));
...
export async function POST(req: Request) {
  ...
  return NextResponse.json({ id: m.id, name: m.name, price: m.price, duration: daysToLabel(m.duration), features: features ?? [], active: true, membersCount: 0 });
}
...
export async function PATCH(req: Request) {
  ...
  return NextResponse.json({
    ...
    duration: daysToLabel(m.duration),
    features: (() => { try { return JSON.parse(m.features); } catch { return []; } })(),
    active: m.isActive, membersCount,
  });
}
