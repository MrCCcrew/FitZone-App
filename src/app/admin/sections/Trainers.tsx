...
const FIELD_LABELS: Record<string, Record<string, string>> = {
  ...
  medical: {
    ...
    takesMeds: "تتناول أدوية",
    medsDetail: "تفاصيل الأدوية",
  },
  injuries: {
    hasInjuries: "لديها إصابات",
    detail: "تفاصيل الإصابات",
    hasSurgeries: "أجرت عمليات",
    surgeryDetail: "تفاصيل العمليات",
  },
  lifestyle: {
    sleepHours: "ساعات النوم",
    sleepQuality: "جودة النوم",
    stressLevel: "مستوى الضغط",
    waterLiters: "شرب الماء (لتر)",
  },
  nutrition: {
    mealsCount: "عدد الوجبات",
    followsDiet: "تتبع نظام غذائي",
    foodAllergies: "حساسية الطعام",
    supplements: "المكملات الغذائية",
  },
  currentActivity: {
    trainingDays: "أيام التمرين أسبوعياً",
    trainingType: "نوع التمرين",
    trainingDuration: "مدة التمرين",
  },
  womensHealth: {
    pregnant: "حامل",
    gaveBirth: "أنجبت",
    lastBirth: "آخر ولادة",
    hasHormonalIssues: "مشاكل هرمونية",
    hormonalIssuesDetail: "تفاصيل المشاكل الهرمونية",
  },
  commitment: {
    goalTimeline: "المدة المستهدفة",
    commitDays: "أيام الالتزام أسبوعياً",
    dietReady: "مستعدة لنظام غذائي",
  },
};
...
function PrivateSessionFormDetails({
  formData,
}: {
    ...
    <div className="mt-4 space-y-4 rounded-xl border border-gray-700 bg-black/20 p-4">
      {Object.entries(formData).map(([section, value]) => {
        ...
        if (section === "notes" || section === "sportsExperience") {
          ...
          return (
            <div key={section}>
              <p className="text-[11px] font-bold text-pink-400 mb-0.5">
                {label}
              </p>
...
type EditableTrainer = Omit<Trainer, "id" | "classesCount"> & {
  id?: string;
  certificationsText: string;
  certificationsEnText: string;
};
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-gray-700 bg-gray-900 p-5">
        <div className="mb-5 flex items-center justify-between">
          <h3 className="text-lg font-black text-white">{title}</h3>
          <button
            onClick={onClose}
            className="text-2xl leading-none text-gray-500 transition-colors hover:text-white"
          >
            ×
          </button>
        </div>
...
export default function Trainers() {
  ...
  const [linkedUserDiscount, setLinkedUserDiscount] = useState<{
    discountType: string;
    discountValue: number;
    maxDiscount: number | null;
  } | null>(null);
  const [activeTab, setActiveTab] = useState<
    "trainers" | "applications" | "discounts" | "attendance" | "referrals"
  >("trainers");

  // Applications
  const [userRole, setUserRole] = useState<string | null>(null);
  const isTrainerRole = userRole === "trainer";
  const isHeadCoachOrAbove =
    userRole === "admin" || userRole === "head_coach" || userRole === "staff";
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [trainersResponse, employeesResponse, sessionResponse] =
        await Promise.all([
      ...
      const employeesPayload = await employeesResponse
        .json()
        .catch(() => ({ employees: [] }));
      ...
      if (sessionPayload?.user?.role)
        setUserRole(sessionPayload.user.role as string);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const loadApplications = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/private-sessions?status=${appStatusFilter}`,
        { cache: "no-store" },
      );
  ...
  const loadAttendance = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/trainer-attendance?${params.toString()}`,
        { cache: "no-store" },
      );
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setApprovePrice("");
        setApproveNote("");
        setApproveSessionsCount("");
        setApproveDurationDays("");
        setApproveSlots([]);
        setApproveSlotInput("");
        setApproveTrainerId("");
        ...
        window.alert((d as { error?: string }).error ?? "حدث خطأ.");
      }
    } finally {
  ...
  const handleCreateDiscount = async () => {
    ...
    try {
      const res = await fetch("/api/admin/trainer-discount-codes", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          trainerId: discountModal.id,
          targetUserId: dcTargetUser,
          discountType: dcType,
          discountValue: Number(dcValue),
          maxDiscount: dcMax ? Number(dcMax) : undefined,
          note: dcNote,
        }),
      });
      ...
      if (res.ok) {
        ...
        setDcError((data as { error?: string }).error ?? "حدث خطأ.");
      }
    } finally {
      setDcSaving(false);
    }
  ...
  const saveTrainer = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        window.alert(
          (payload.error ?? "تعذر حفظ بيانات المدربة الآن.") +
            (detail ? `\n\nالتفاصيل: ${detail}` : ""),
        );
        return;
      }
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      setUploadError(
        error instanceof Error ? error.message : "تعذر رفع صورة المدربة الآن.",
      );
    } finally {
      setUploading(false);
    }
  ...
  const uploadCertificate = async (file: File) => {
    ...
    try {
        ...
        current
          ? {
              ...
              certificateFiles: [
                ...
                { url: payload.url as string, label },
              ],
            }
          : current,
      );
      ...
      setUploadError(
        error instanceof Error ? error.message : "تعذر رفع صورة الشهادة الآن.",
      );
    } finally {
      setUploading(false);
    }
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center text-sm text-gray-500">
        جارٍ تحميل بيانات المدربات...
      </div>
    );
  }

  const activeCount = trainers.filter((trainer) => trainer.active).length;
  const visibleOnHomeCount = trainers.filter(
    (trainer) => trainer.showOnHome && trainer.active,
  ).length;
  const totalClasses = trainers.reduce(
    (sum, trainer) => sum + trainer.classesCount,
    0,
  );
  const totalSessions = trainers.reduce(
    (sum, trainer) => sum + trainer.sessionsCount,
    0,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-gray-800 pb-1">
        {(
          [
            ["trainers", "المدربات"],
            ...(!isTrainerRole
              ? ([["applications", "طلبات البرايفيت"]] as const)
              : []),
            ["discounts", "أكواد خصم المدربات"],
            ...(canManageTrainerReferrals
              ? ([["referrals", "لينكات الإحالة"]] as const)
              : []),
          ] as [string, string][]
        ).map(([key, label]) => (
          <button
        ...
        <section className="space-y-4">
          <div className="flex flex-wrap items-center gap-3">
            <h3 className="text-lg font-black text-white">
              طلبات البرايفيت والميني برايفيت
            </h3>
          ...
          {appLoading ? (
            ...
            <div className="py-10 text-center text-sm text-gray-500">
              لا توجد طلبات
              {appStatusFilter !== "all"
                ? ` بحالة "${STATUS_LABELS[appStatusFilter] ?? appStatusFilter}"`
                : ""}
            </div>
          ) : (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs font-bold ${app.type === "private" ? "bg-pink-900/40 text-pink-300" : "bg-purple-900/40 text-purple-300"}`}
                        >
                          {app.type === "private" ? "برايفيت" : "ميني برايفيت"}
                        </span>
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs font-bold ${STATUS_COLORS[app.status] ?? "text-gray-400"}`}
                        >
                      ...
                      {app.goals.length > 0 && (
                        <div className="text-xs text-gray-400">
                          الأهداف: {app.goals.join("، ")}
                        </div>
                      )}
                      {app.trainerPrice && (
                        <div className="text-xs font-bold text-emerald-300">
                          السعر المحدد: {app.trainerPrice} ج.م
                        </div>
                      )}
                      {(app.sessionsCount || app.durationDays) && (
                        <div className="text-xs text-blue-300">
                          ...
                          {app.expiresAt
                            ? ` · تنتهي ${new Date(app.expiresAt).toLocaleDateString("ar-EG")}`
                            : ""}
                        </div>
                      )}
                      {app.trainerNote && (
                        <div className="text-xs text-gray-400">
                          ملاحظة: {app.trainerNote}
                        </div>
                      )}
                    </div>
                        ...
                        <>
                          <button
                            onClick={() => {
                              ...
                              setApprovePrice("");
                              setApproveNote("");
                              setApproveTrainerId(app.trainer.id);
                              setApproveSlots([]);
                              setApproveSlotInput("");
                              setApproveSessionsCount("");
                              setApproveDurationDays("");
                          ...
                          <button
                            ...
                            className="rounded-lg bg-red-900/60 px-3 py-1.5 text-xs font-bold text-red-300"
                          >
                            ✕ رفض
                          </button>
                        </>
      ...
      {activeTab === "discounts" && (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-black text-white">
              أكواد خصم المدربات للعملاء المميزين
            </h3>
            <button
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800 text-right text-xs text-gray-500">
                    <th className="pb-2 pr-2">الكود</th>
                    <th className="pb-2">المدربة</th>
                ...
                <tbody className="divide-y divide-gray-800">
                  {discountCodes.map((c) => (
                    <tr key={c.id} className="text-xs">
                      <td className="py-2 pr-2 font-mono font-bold text-pink-300">
                        {c.code}
                      </td>
                      ...
                      <td className="py-2 text-gray-300">
                        {c.clientName ?? "—"}
                        <br />
                        <span className="text-gray-500">{c.clientEmail}</span>
                      </td>
                      ...
                      <td className="py-2">
                        <span
                          className={`rounded-full px-2 py-0.5 font-bold ${c.isUsed ? "bg-gray-800 text-gray-400" : "bg-emerald-900/30 text-emerald-300"}`}
                        >
                      ...
                      <td className="py-2">
                        <button
                          onClick={async () => {
                            ...
                            await fetch(
                              `/api/admin/trainer-discount-codes?id=${c.id}`,
                              { method: "DELETE" },
                            );
                            void loadDiscounts();
                          }}
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              onClick={() =>
                setAttModal({
                  trainerId: attendanceTrainerId || (trainers[0]?.id ?? ""),
          ...
          {attendanceLoading ? (
            ...
            <div className="overflow-x-auto rounded-2xl border border-gray-800">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800 bg-gray-900/60 text-right text-xs text-gray-500">
                      ...
                      <td className="px-4 py-3 flex gap-2">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            await fetch(
                              `/api/admin/trainer-attendance?id=${log.id}`,
                              { method: "DELETE" },
                            );
                            void loadAttendance();
                          }}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
              <div className="w-full max-w-md rounded-2xl border border-white/10 bg-gray-900 p-6 space-y-4">
                <h4 className="text-base font-black text-white">
                  {attModal.id ? "تعديل سجل الحضور" : "تسجيل حضور / غياب"}
                </h4>
                <div className="space-y-3">
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setAttModal({ ...attModal, trainerId: e.target.value })
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      value={attModal.date}
                      onChange={(e) =>
                        setAttModal({ ...attModal, date: e.target.value })
                      }
                      className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-white"
                    />
                  </div>
                    ...
                    <div className="flex gap-2">
                      {(
                        ...
                        <button
                          key={val}
                          onClick={() =>
                            setAttModal({ ...attModal, status: val })
                          }
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setAttModal({ ...attModal, notes: e.target.value })
                      }
                      rows={2}
                      className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-white"
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    onClick={async () => {
                      ...
                      try {
                        await fetch("/api/admin/trainer-attendance", {
                          ...
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                            trainerId: attModal.trainerId,
                            date: attModal.date,
                            status: attModal.status,
                            notes: attModal.notes || undefined,
                          }),
                        });
                        setAttModal(null);
                        void loadAttendance();
                  ...
                  >
                    {attSaving ? "جارٍ الحفظ..." : "حفظ"}
                  </button>
                  <button
                    onClick={() => setAttModal(null)}
                    className="flex-1 rounded-xl border border-white/10 py-2.5 text-sm font-bold text-gray-300 hover:text-white"
                  >
                    إلغاء
                  </button>
                </div>
      ...
      {activeTab === "trainers" && (
        <>
          <section className="grid gap-4 lg:grid-cols-[1.4fr,1fr,1fr,1fr]">
            <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5">
              ...
              <div className="mt-3 text-xs leading-6 text-gray-500">
                من هنا تدير بيانات المدربات كاملة: الصورة، التخصص، النبذة،
                الشهادات، ترتيب الظهور، والحالة. أما نصوص صفحة المدربات العامة
                فستجدها داخل قسم
                <span className="px-1 font-bold text-pink-300">
                  الصفحات والمحتوى → صفحة المدربات
                </span>
                .
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5">
              ...
              <div className="mt-3 text-xs text-gray-500">
                المدربات المتاحات حاليًا داخل الموقع ولوحة الإدارة.
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5">
              ...
              <div className="mt-3 text-xs text-gray-500">
                عدد المدربات المعروضات في الصفحة الرئيسية.
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5">
              ...
              <div className="mt-3 text-xs text-gray-500">
                مؤشرات سريعة لمتابعة حجم عمل فريق المدربات.
              </div>
            </div>
          </section>
            ...
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs leading-6 text-gray-500">
                  ...
                  بخلفية نظيفة وقص يركز على الوجه والكتفين لتظهر بشكل أنيق في
                  صفحة المدربات والرئيسية.
                </p>
              </div>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              {filtered.map((trainer) => (
                ...
                >
                        ...
                        <div>
                          ...
                          {trainer.linkedUser ? (
                            <div className="mt-1 text-xs text-emerald-300">
                              {trainer.linkedUser.name || "حساب مدربة"} ·{" "}
                              {trainer.linkedUser.email}
                            </div>
                          ) : (
                  ...
                  {trainer.certificateFiles &&
                  trainer.certificateFiles.length ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <div className="mt-4 flex gap-2">
                    <button
                      onClick={() => {
                        ...
                        setModal({
                          ...
                          certificationsEnText: listToText(
                            trainer.certificationsEn ?? [],
                          ),
                        });
                      }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        setDcError("");
                        setDcType(
                          trainer.linkedUser?.discountType ?? "percentage",
                        );
                        setDcValue(
                          trainer.linkedUser?.discountValue
                            ? String(trainer.linkedUser.discountValue)
                            : "",
                        );
                        setDcMax(
                          trainer.linkedUser?.maxDiscount
                            ? String(trainer.linkedUser.maxDiscount)
                            : "",
                        );
                        void loadMembers();
                      }}
          ...
          {modal ? (
            <Modal
              title={modal.id ? "تعديل بيانات المدربة" : "إضافة مدربة جديدة"}
              onClose={() => setModal(null)}
            >
              <div className="space-y-4">
                <div className="rounded-2xl border border-gray-800 bg-black/20 p-4 text-xs leading-7 text-gray-400">
                  أدخلي البيانات كما ستظهر للعميلة داخل صفحة المدربات والصفحة
                  الرئيسية. يفضل أن تكون الصورة شخصية أو نصفية، وأن تكتب
                  الشهادات كل شهادة في سطر مستقل لتظهر بشكل مرتب.
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {!isTrainerRole && (
                    <FieldHint
                      title="ربط الحساب"
                      hint="اختاري حساب المدربة حتى تتمكن من تعديل ملفها من داخل حسابها."
                    >
                      <select
                        value={modal.userId ?? ""}
                        onChange={(event) =>
                  ...
                  <FieldHint
                    title="اسم المدربة"
                    hint="الاسم الظاهر للعميلات داخل صفحة المدربات."
                  >
                    <input
                      value={modal.name}
                      onChange={(event) =>
                  ...
                  >
                    <div className="flex gap-2">
                      <input
                        value={modal.nameEn ?? ""}
                        onChange={(event) =>
                          setModal({ ...modal, nameEn: event.target.value })
                        }
                      ...
                      <TranslateButton
                        from={modal.name}
                        onTranslated={(t) => setModal({ ...modal, nameEn: t })}
                      />
                    </div>
                  </FieldHint>
                  ...
                  <FieldHint
                    title="التخصص"
                    hint="مثال: يوجا وتأهيل بدني أو زومبا وكارديو."
                  >
                    <input
                      value={modal.specialty}
                      onChange={(event) =>
                        setModal({ ...modal, specialty: event.target.value })
                      }
                      className={INPUT}
                    />
                  </FieldHint>
                  ...
                  >
                    <div className="flex gap-2">
                      ...
                      />
                      <TranslateButton
                        from={modal.specialty}
                        onTranslated={(t) =>
                          setModal({ ...modal, specialtyEn: t })
                        }
                      />
                    </div>
                  </FieldHint>
                  ...
                  <FieldHint
                    title="التقييم"
                    hint="يظهر على شكل 4.9 أو 5.0 داخل البطاقة."
                  >
                    <input
                      type="number"
                      step="0.1"
                  ...
                  <FieldHint
                    title="عدد الجلسات"
                    hint="إحصائية تعريفية تظهر للعميلة داخل صفحة المدربات."
                  >
                    <input
                      type="number"
                      ...
                      onChange={(event) =>
                        setModal({
                          ...modal,
                          sessionsCount: Number(event.target.value) || 0,
                        })
                      }
                      className={INPUT}
                    />
                  ...
                  {!isTrainerRole && (
                    <FieldHint
                      title="ترتيب الظهور"
                      hint="الأقل يظهر أولًا في الصفحة الرئيسية وصفحة المدربات."
                    >
                      <input
                        type="number"
                      ...
                      <label className="rounded-xl border border-gray-700 bg-gray-800 p-3 text-sm text-white">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setModal({
                              ...modal,
                              showOnHome: event.target.value === "show",
                            })
                          }
                          className={INPUT}
                        >
                ...
                >
                  <textarea
                    value={modal.bio ?? ""}
                    onChange={(event) =>
                      setModal({ ...modal, bio: event.target.value })
                    }
                    rows={4}
                ...
                >
                  <div className="space-y-1">
                    <textarea
                      value={modal.bioEn ?? ""}
                      onChange={(event) =>
                        setModal({ ...modal, bioEn: event.target.value })
                      }
                      rows={4}
                      className={`${INPUT} resize-none`}
                      dir="ltr"
                    ...
                    <TranslateButton
                      from={modal.bio ?? ""}
                      onTranslated={(t) => setModal({ ...modal, bioEn: t })}
                    />
                  </div>
                </FieldHint>
                  ...
                  <div className="space-y-1">
                    <textarea
                      ...
                      onChange={(event) =>
                        setModal({
                          ...modal,
                          certificationsEnText: event.target.value,
                        })
                      }
                      rows={4}
                      className={`${INPUT} resize-none`}
                    ...
                    <TranslateButton
                      ...
                      onTranslated={(t) =>
                        setModal({ ...modal, certificationsEnText: t })
                      }
                    />
                  </div>
                </FieldHint>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <input
                      value={modal.image ?? ""}
                      onChange={(event) =>
                        setModal({ ...modal, image: event.target.value })
                      }
                  ...
                  {modal.image ? (
                    <div className="max-w-[220px] overflow-hidden rounded-2xl border border-gray-700 bg-gray-800">
                      <img
                        src={modal.image}
                        alt={modal.name || "trainer-preview"}
                  ...
                  {modal.certificateFiles && modal.certificateFiles.length ? (
                    <div className="grid gap-3 md:grid-cols-2">
                      {modal.certificateFiles.map((file, index) => (
                        <div
                          key={`${file.url}-${index}`}
                          className="rounded-2xl border border-gray-700 bg-gray-800/60 p-3"
                        >
                          <div className="mb-3 overflow-hidden rounded-xl border border-gray-700 bg-black/20">
                            <img
                              src={file.url}
                              alt={file.label || `certificate-${index + 1}`}
                ...
                {!isTrainerRole && (
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-950/20 p-4 space-y-3">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <label className="rounded-xl border border-gray-700 bg-gray-800 p-3 text-sm text-white">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setModal({
                              ...modal,
                              canSendGifts: event.target.value === "yes",
                            })
                          }
                          className={INPUT}
                        >
                      ...
                      {modal.canSendGifts && (
                        <FieldHint
                          title="الحد الشهري للهدايا"
                          hint="أقصى عدد كلاسات هدية يمكن للمدربة إرسالها شهريًا."
                        >
                          <input
                            type="number"
                            min={0}
                      ...
                      <label className="rounded-xl border border-gray-700 bg-gray-800 p-3 text-sm text-white">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setModal({
                              ...modal,
                              canAddClasses: event.target.value === "yes",
                            })
                          }
                          className={INPUT}
                        >
                      ...
                      <label className="rounded-xl border border-gray-700 bg-gray-800 p-3 text-sm text-white">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setModal({
                              ...modal,
                              canAddBookings: event.target.value === "yes",
                            })
                          }
                          className={INPUT}
                        >
                ...
                {modal?.linkedUser && linkedUserDiscount && (
                  <div className="rounded-2xl border border-emerald-500/20 bg-emerald-950/20 p-4 space-y-3">
                    <div className="text-sm font-black text-emerald-300">
                      إعداد خصم أكواد المدربة
                    </div>
                    <p className="text-xs text-gray-400">
                      القيمة التي تستخدمها المدربة عند إنشاء أكواد خصم لعملائها
                      — لا تستطيع المدربة تغييرها.
                    </p>
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-1">
                        <label className="text-xs text-gray-400">
                      ...
                      <div className="space-y-1">
                        <label className="text-xs text-gray-400">
                          {linkedUserDiscount.discountType === "fixed"
                            ? "مبلغ الخصم ج.م"
                            : "نسبة الخصم %"}
                        </label>
                        <input
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
              <div className="w-full max-w-md rounded-2xl border border-gray-700 bg-gray-900 p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                ...
                <div className="text-sm text-gray-400">
                  العميل:{" "}
                  <span className="text-white">{approveModal.user.name}</span> —{" "}
                  {approveModal.type === "private" ? "برايفيت" : "ميني برايفيت"}
                </div>
                ...
                {isAdmin && (
                  <div>
                    ...
                    {approveTrainerId &&
                      approveTrainerId !== approveModal.trainer.id && (
                        <p className="text-xs text-amber-400 mt-1">
                          سيتم تغيير المدربة من "{approveModal.trainer.name}"
                          إلى المدربة المختارة
                        </p>
                      )}
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={approveSessionsCount}
                      onChange={(e) => setApproveSessionsCount(e.target.value)}
                      className={INPUT}
                      placeholder="مثال: 12"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-400 mb-1">
                      المدة بالأيام (اختياري)
                    </label>
                    <input
                      type="number"
                      value={approveDurationDays}
                      onChange={(e) => setApproveDurationDays(e.target.value)}
                      className={INPUT}
                      placeholder="مثال: 30"
                    />
                  </div>
                ...
                <div className="rounded-xl border border-pink-500/20 bg-pink-900/10 p-4 space-y-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      أضف المواعيد المناسبة — العميل سيختار منها قبل الدفع.
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          ...
                          if (v && !approveSlots.includes(v)) {
                            setApproveSlots([...approveSlots, v]);
                            setApproveSlotInput("");
                          }
                        }
                      }}
                  ...
                  {approveSlots.length > 0 ? (
                    <div className="space-y-1">
                      {approveSlots.map((slot) => (
                        <div
                          key={slot}
                          className="flex items-center justify-between rounded-lg bg-gray-800 px-3 py-2 text-sm text-white"
                        >
                          <span>📌 {slot}</span>
                          <button
                            type="button"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className={`${INPUT} resize-none`}
                    placeholder="مثال: موعد البداية الأحد القادم..."
                  />
                </div>
                <div className="flex gap-2">
                  <button
                  ...
                  <button
                    ...
                    className="rounded-xl bg-gray-800 px-4 py-2.5 font-bold text-gray-300"
                  >
                    إلغاء
                  </button>
                </div>
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-gray-700 bg-gray-900 p-6 space-y-4">
            ...
            <div className="text-sm text-gray-400">
              العميل:{" "}
              <span className="text-white">{rejectModal.user.name}</span> —{" "}
              {rejectModal.type === "private" ? "برايفيت" : "ميني برايفيت"} مع{" "}
              <span className="text-pink-300">{rejectModal.trainer.name}</span>
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">
                سبب الرفض (اختياري)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-gray-700 bg-gray-900 p-6 space-y-4">
            ...
            <div>
              ...
              <select
                value={dcTargetUser}
                onChange={(e) => setDcTargetUser(e.target.value)}
                className={INPUT}
              >
            ...
            {discountModal.linkedUser &&
            discountModal.linkedUser.discountValue > 0 ? (
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-950/20 p-3 space-y-2">
                <div className="text-xs font-bold text-gray-400">
                  قيم الخصم المعتمدة للمدربة (غير قابلة للتعديل)
                </div>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="rounded-xl border border-amber-500/20 bg-amber-950/20 px-4 py-3 text-xs text-amber-300">
                ⚠️ لم يتم تعيين قيم خصم لهذه المدربة. يرجى تعديل بيانات المدربة
                أولًا وتحديد نوع وقيمة الخصم.
              </div>
            )}
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-1">
            ...
            {dcError && (
              <div className="rounded-xl bg-red-950/40 border border-red-500/30 px-4 py-2 text-xs text-red-300">
                {dcError}
              </div>
            )}
            <div className="flex gap-2">
              <button
                ...
                disabled={dcSaving || !dcTargetUser || !dcValue}
