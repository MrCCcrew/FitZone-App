...
vi.mock("@/lib/analytics/privacy", () => ({
  isAnalyticsBot: vi.fn(() => false),
  sanitizeAnalyticsPath: vi.fn((value: string) =>
    value.startsWith("/admin") ? null : value.split("?")[0]
  ),
...
describe("analytics page-view collector", () => {
  ...
  it("ignores untrusted body IDs", async () => { const req = new Request("http://localhost/api/analytics/collect", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ eventName: "page_view", path: "/", userId: "bad", visitorId: "bad", sessionId: "bad" }) }); await POST(req); expect(getOrCreateAnalyticsVisitor).toHaveBeenCalledWith(
      undefined,
      null,
  ...
  it("links authenticated server identity to visitor, session and page view", async () => {
    vi.mocked(getCurrentAppUser).mockResolvedValueOnce({ id: "u1" } as never);

    await POST(request("/account"));

    expect(getOrCreateAnalyticsVisitor).toHaveBeenCalledWith(
      undefined,
      "u1",
      expect.any(Object),
    );

    expect(getOrCreateAnalyticsSession).toHaveBeenCalledWith(
      undefined,
      "v",
      "u1",
      expect.any(Date),
    ...
    expect(db.analyticsPageView.create).toHaveBeenCalledWith({
