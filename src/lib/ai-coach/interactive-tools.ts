...
export async function searchSchedulesInteractive(query = ""): Promise<InteractiveToolResult> {
  try {
    ...
    return interactiveToolResultSchema.parse({ status: state === "ok" && matched.length ? "success" : "empty", sourceType: "Class + Schedule", resultType: "schedule", data: matched.map((row) => ({ id: row.id, classId: row.classId, name: row.class.name, trainer: row.class.trainer.name, date: row.date.toISOString(), time: row.time, availableSpots: row.availableSpots })), resultCount: matched.length, spokenSummary: summary, cards: matched.slice(0, 3).map((row) => ({ id: row.id, title: row.class.name, subtitle: `${row.time}`, image: row.class.image })), navigationTarget: nav("classes"), warnings: state === "ok" ? [] : [state] });
  } catch {
    traceCoachCatalogRead({ sourceStatus: "source_error" });
