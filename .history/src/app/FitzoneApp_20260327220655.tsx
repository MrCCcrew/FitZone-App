    ...
    trainer1: {
      content: (
        <>
          ...
          <text x="50%" y="98%" textAnchor="middle" fontSize="8" fill="#e776a3" fontFamily="'Cairo',sans-serif">مدربة رئيسية</text>
        </>
      )
    },
    trainer2: {
      content: (
        <>
          ...
          <text x="50%" y="98%" textAnchor="middle" fontSize="8" fill="#8B5CF6" fontFamily="'Cairo',sans-serif">مدربة يوجا</text>
        </>
      )
    },
    trainer3: {
      content: (
        <>
          ...
          <text x="50%" y="98%" textAnchor="middle" fontSize="8" fill="#0D9488" fontFamily="'Cairo',sans-serif">مدربة قوة</text>
        </>
      )
    },
    product1: {
            ...
            {testimonials.map(t => (
              <div key={t.name} className="card" style={{ padding: 28, borderRight: `3px solid ${C.red}` }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 38, height: 38, background: "rgba(233,30,99,.2)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: C.red, fontWeight: 700 }}>{t.name[0]}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: responsiveColumns("1fr", "1fr", "1fr 1fr"), gap: 48, alignItems: "start" }}>
          ...
          <div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {sizes.map(s => (
                  <button key={s} onClick={() => setSelectedSize(s)} style={{ minWidth: 42, height: 42, padding: "0 10px", borderRadius: 8, border: selectedSize === s ? `2px solid ${C.red}` : `1px solid ${C.border}`, background: selectedSize === s ? "rgba(233,30,99,.15)" : C.bgCard2, color: selectedSize === s ? C.red : C.gray, fontWeight: 700, cursor: "pointer", fontFamily: "'Cairo', sans-serif", fontSize: 13 }}>{s}</button>
                ))}
              </div>
            </div>
