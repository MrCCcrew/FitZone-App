...
export async function POST(req: Request) {
  ...
  if (!membershipId) {
    return NextResponse.json({ error: "يرجى اختيار باقةا" }, { status: 400 });
  }

  // تحقق من تفعيل البريد الإلكتروني
  const userRecord = await db.user.findUnique({ where: { id: userId }, select: { emailVerified: true } });
  if (!userRecord?.emailVerified) {
    return NextResponse.json({ error: "يجب تفعيل حسابك أولاً قبل الاشتراك", needsVerification: true }, { status: 403 });
  }
