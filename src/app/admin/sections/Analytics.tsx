...
import { analyticsQuery, loadAdminAnalytics, type AnalyticsFilters } from "@/lib/analytics/admin-client";
...
const duration = (value: number) => `${Math.floor(value / 60)}m ${Math.round(value % 60)}s`;
...
export default function Analytics() {
  ...
  const load = useCallback(async (next = filters) => {
    ...
    try {
      ...
      const payload: typeof data = {}; const failed: string[] = [];
      for (const [name, item] of Object.entries(result)) { if (item.status === "fulfilled") payload[name as keyof typeof payload] = item.value as never; else failed.push(name); }
      setData(payload); setErrors(failed.length ? [`تعذر تحميل: ${failed.join("، ")}`] : []);
  ...
  const funnel = useMemo(() => conversions ? [{ name: "Views", count: conversions.membershipFunnel.views }, { name: "Checkout", count: conversions.membershipFunnel.checkoutStarted }, { name: "Paid", count: conversions.membershipFunnel.paymentSucceeded }, { name: "Activated", count: conversions.membershipFunnel.membershipActivated }] : [], [conversions]);
  return <AdminSectionShell title="تحليلات الموقع" subtitle="مؤشرات مجمعة، مع Funnel مبني على الأحداث وليس على مستخدمين فريدين." actions={<button onClick={() => void load(filters)} className="rounded-xl bg-pink-600 px-4 py-2 text-sm font-bold text-white">إعادة المحاولة</button>}>
    ...
    {loading && !overview ? <AdminCard className="h-52 animate-pulse bg-white/10"><span className="sr-only">جارٍ التحميل</span></AdminCard> : <>
      {overview ? <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="الزوار" value={count(overview.traffic.visitors)} /><Metric label="الجلسات" value={count(overview.traffic.sessions)} /><Metric label="مشاهدات الصفحة" value={count(overview.traffic.pageViews)} /><Metric label="معدل الارتداد" value={rate(overview.traffic.bounceRate)} /><Metric label="متوسط الجلسة" value={duration(overview.traffic.averageSessionDuration)} /><Metric label="بدء الدفع" value={count(overview.business.checkoutStarted)} /><Metric label="دفع ناجح" value={count(overview.business.paymentSucceeded)} /><Metric label="عضويات مفعلة" value={count(overview.business.membershipActivated)} /></div><div className="grid gap-4 lg:grid-cols-2">{overview.revenue.currencyBreakdown.length ? overview.revenue.currencyBreakdown.map((entry) => <Metric key={entry.currency} label={`إيراد ناجح (${entry.currency})`} value={`${count(entry.value)} ${entry.currency}`} hint={`المتوسط: ${count(entry.averageValue)} ${entry.currency}`} />) : <AdminCard><AdminEmptyState title="لا توجد إيرادات" description="لا توجد مدفوعات ناجحة ضمن الفترة." /></AdminCard>}</div></> : null}
      {traffic ? <AdminCard><h3 className="mb-4 font-black">الزيارات اليومية</h3><div className="h-72" dir="ltr"><ResponsiveContainer><LineChart data={traffic.daily}><CartesianGrid strokeDasharray="3 3" stroke="#ffffff22" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Legend /><Line type="monotone" dataKey="visitors" stroke="#ff4f93" /><Line type="monotone" dataKey="sessions" stroke="#f8b94d" /><Line type="monotone" dataKey="pageViews" stroke="#8b5cf6" /></LineChart></ResponsiveContainer></div></AdminCard> : null}
      {conversions ? <div className="grid gap-4 lg:grid-cols-2"><AdminCard><h3 className="font-black">مسار العضوية</h3><p className="mt-1 text-xs text-[#d7aabd]">Event-based funnel</p><div className="h-64" dir="ltr"><ResponsiveContainer><BarChart data={funnel}><XAxis dataKey="name" /><YAxis /><Tooltip /><Bar dataKey="count" fill="#ff4f93" radius={[8,8,0,0]} /></BarChart></ResponsiveContainer></div><div className="grid grid-cols-3 gap-2 text-center text-xs"><span>View→Checkout {rate(conversions.membershipFunnel.viewToCheckoutRate)}</span><span>Checkout→Payment {rate(conversions.membershipFunnel.checkoutToPaymentRate)}</span><span>Payment→Activation {rate(conversions.membershipFunnel.paymentToActivationRate)}</span></div></AdminCard><AdminCard><h3 className="font-black">مسار المتجر المنفصل</h3><p className="mt-4 text-sm">Order Checkout: <b>{count(conversions.storeFunnel.checkoutStarted)}</b></p><p className="mt-2 text-sm">Order Payment Success: <b>{count(conversions.storeFunnel.paymentSucceeded)}</b></p><p className="mt-2 text-xs text-[#d7aabd]">التحويل: {rate(conversions.storeFunnel.totalRate)}</p></AdminCard></div> : null}
      ...
      {events ? <div className="grid gap-4 xl:grid-cols-2"><SimpleTable title="إجمالي الأحداث" columns={["الحدث","العدد"]} rows={events.totalsByEventName.map((entry) => [entry.eventName, count(entry.count)])} /><SimpleTable title="أهم الكيانات" columns={["النوع","الاسم","العدد","قيمة ناجحة"]} rows={events.topEntities.map((entry) => [entry.entityType ?? "—", entry.entityName ?? "—", count(entry.count), count(entry.successfulValue)])} /><SimpleTable title="طرق الدفع" columns={["الطريقة","العدد"]} rows={events.paymentMethodBreakdown.map((entry) => [entry.paymentMethodType, count(entry.count)])} /><SimpleTable title="فئات الفشل" columns={["الفئة","العدد"]} rows={events.failureCategoryBreakdown.map((entry) => [entry.failureCategory, count(entry.count)])} /></div> : null}
    </>}
  </AdminSectionShell>;
}
