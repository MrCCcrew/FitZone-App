...
import { configuredTtsVoice, configuredVoiceSpeed, ttsEnabled } from "@/lib/ai-coach/voice/config";
...
export async function POST(req: Request) {
  if (!ttsEnabled()) return NextResponse.json({ error: "Voice playback is unavailable." }, { status: 503 });
  ...
  try {
    ...
    const speech = await openAiTtsProvider.synthesize({ text: selectVoiceText(message.content, message.metadata), voice: configuredTtsVoice(), speed });
    return new NextResponse(speech.audio, { headers: { "Content-Type": speech.contentType, "Cache-Control": "private, max-age=300" } });
