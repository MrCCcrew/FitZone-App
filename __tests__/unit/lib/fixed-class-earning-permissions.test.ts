...
describe("Fixed Class Earning permissions", () => {
  const view = "fixed_class_earning_view" as const;
  const calculate = "fixed_class_earning_calculate" as const;
  const finalize = "fixed_class_earning_finalize" as const;
  ...
  it("permissions remain independent from each other", () => {
    const user = {
      role: "staff",
  ...
  it("admin has all fixed-class earning permissions", () => {
    const admin = {
      role: "admin",
      permissions: [],
    };
