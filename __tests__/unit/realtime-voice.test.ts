...
describe("Realtime voice session", () => {
  ...
  it("issues only an ephemeral client secret and configures read-only tools", async () => {
    ...
    expect(body.session.audio.input.turn_detection).toEqual({ type: "server_vad", threshold: 0.25, prefix_padding_ms: 500, silence_duration_ms: 900, create_response: false, interrupt_response: false });
