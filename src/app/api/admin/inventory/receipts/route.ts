...
import { relinkInventoryReceipt } from "@/lib/supplier-ap-service";
...
export async function GET() {
  ...
  return NextResponse.json(
    receipts.map((receipt) => ({
      ...
      purchaseInvoiceId: receipt.purchaseInvoiceId ?? null,
...
export async function POST(req: Request) {
  ...
  if (!items.length) {
    return NextResponse.json(
      { error: "يرجى إضافة منتجات للمشتريات." },
      { status: 400 },
    );
  }
  ...
  const incomingReceiptTotalMinor = sanitizedItems.reduce(
    (sum, item) =>
      sum + Math.round(Number(item.unitCost) * Number(item.quantity) * 100),
    0,
  );

  const purchaseInvoiceId = body.purchaseInvoiceId
    ? String(body.purchaseInvoiceId).trim()
    : null;
  ...
  const result = await db
    .$transaction(async (tx) => {
      let resolvedSupplierId = body.supplierId
        ? String(body.supplierId).trim()
        : null;

      if (purchaseInvoiceId) {
        // Same lock order as postPurchaseInvoice():
        // PurchaseInvoice first, then receipt rows.
        await tx.$queryRaw(
          Prisma.sql`
            SELECT id
        ...
        if (!purchaseInvoice) {
          throw new Error("فاتورة المورد غير موجودة");
        }

        if (purchaseInvoice.status !== "draft") {
          throw new Error(
            "يمكن ربط استلام المخزون بفواتير الموردين المسودة فقط",
          );
        }
        ...
        if (
          ...
          throw new Error(
            "المورد في استلام المخزون لا يطابق مورد فاتورة الشراء",
          );
        }

        resolvedSupplierId = purchaseInvoice.supplierId;

        const existingLinkedReceipts = await tx.inventoryReceipt.findMany({
          where: {
            purchaseInvoiceId,
            status: "posted",
          },
        ...
        const alreadyReceivedMinor = existingLinkedReceipts.reduce(
          (sum, existingReceipt) =>
            sum + Math.round(Number(existingReceipt.totalCost) * 100),
          0,
        );

        const invoiceTotalMinor = Math.round(
          Number(purchaseInvoice.totalAmount) * 100,
        );

        if (
          alreadyReceivedMinor + incomingReceiptTotalMinor >
          invoiceTotalMinor
        ) {
          throw new Error(
            `إجمالي الاستلامات سيتجاوز قيمة فاتورة المورد. ` +
              `قيمة الفاتورة: ${(invoiceTotalMinor / 100).toFixed(2)}، ` +
              `المستلم سابقًا: ${(alreadyReceivedMinor / 100).toFixed(2)}، ` +
              `الاستلام الحالي: ${(incomingReceiptTotalMinor / 100).toFixed(2)}`,
          );
        }
      }
    ...
    .catch((error: unknown) => {
      const message =
        error instanceof Error ? error.message : "تعذر تسجيل المشتريات.";
...
export async function PATCH(req: Request) {
  ...
  if (action === "relink_purchase_invoice") {
    if (!body.newPurchaseInvoiceId?.trim()) {
      return NextResponse.json(
        { error: "فاتورة المورد الجديدة مطلوبة" },
        { status: 400 },
      );
    }
    ...
    try {
      const result = await relinkInventoryReceipt({
        receiptId: id,
        newPurchaseInvoiceId: body.newPurchaseInvoiceId.trim(),
      ...
      await logAudit({
        ...
        targetId: id,
        details: {
          oldPurchaseInvoiceId: result.fromPurchaseInvoiceId,
          newPurchaseInvoiceId: result.toPurchaseInvoiceId,
      ...
      const message =
        error instanceof Error
          ? error.message
          : "تعذر تغيير فاتورة المورد المرتبطة";
  ...
  if (action === "void") {
    ...
    try {
      await db.$transaction(
        async (tx) => {
          const receiptSnapshot = await tx.inventoryReceipt.findUnique({
            where: { id },
            select: {
              id: true,
          ...
          // Lock order is PurchaseInvoice -> InventoryReceipt.
          // postPurchaseInvoice() uses the same order to prevent a race between
          // posting the financial invoice and voiding its inventory receipt.
          if (receiptSnapshot.purchaseInvoiceId) {
            await tx.$queryRaw(
              Prisma.sql`
              SELECT id
              FROM PurchaseInvoice
              WHERE id = ${receiptSnapshot.purchaseInvoiceId}
              FOR UPDATE
            `,
            );
          }
          ...
          if (receipt.purchaseInvoice?.status === "posted") {
            throw new Error(
              "لا يمكن إلغاء استلام المخزون لأنه مرتبط بفاتورة مورد مرحلة. ألغِ المعالجة المالية للفاتورة أولاً.",
            );
          }
          ...
          for (const item of receipt.items) {
            const downstreamMovements = await tx.inventoryMovement.count({
              where: {
                ...
                // Do not treat this receipt's own purchase movement as downstream.
                // It is created immediately after the receipt header itself.
                NOT: {
                  referenceType: "inventory_receipt",
                  referenceId: receipt.id,
                },
          ...
          for (const item of receipt.items) {
            ...
            if (stockAfter > 0) {
              ...
              newAvg =
                (stockBefore * avgBefore - item.quantity * item.unitCost) /
                stockAfter;
      ...
      const message =
        error instanceof Error ? error.message : "تعذر إلغاء الإيصال";
      return NextResponse.json({ error: message }, { status: 400 });
    }
  }
